...
<?php if( is_single() && has_term( 'patient', 'category' ) ) { ?> 
	...
	$display_shortcode_tax = '[display-posts post_type="studies" taxonomy="indications" tax_term="' . $term_slug . '" wrapper="div" wrapper_class="display-posts-listing grid-x small-up-1 medium-up-3 grid-margin-x grid-padding-x" posts_per_page="3" order="ASC" orderby="date" image_size="thumbnail" exclude_current="true"]';
		...
		<div class="content grid-container page-content">
			<div class="inner-content grid-x grid-margin-x grid-padding-x">
			    <div class="small-12 medium-10 medium-offset-1 text-center cell">
			    	...
			    	<?php if( $execute_code != '' ) { ?>
			    	<p>You might be interested in these related Studies</p>
					<?php echo $execute_code; ?>
				<?php } else { ?>
			    	<p>There are no Related Studies at this time</p>
				<?php } ?>
					<a href="<?php echo site_url(); ?>/studies" class="orange-button">View All Studies</a>
				</div> 
			</div> 
		</div> 
	</div>
